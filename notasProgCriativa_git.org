...
- Desafio:
  - Investigar a documentação e descobrir:
    ...
    - Criar um exemplo que demonstre o funcionamento de /cada uma/ dessas funções

- Observação:
  - Ao navegar a documentação dessas funções, provavelmente irá encontrar algo chamado "/if statement/", escrito assim: ~if()~. Não é necessário o seu uso. Isso será assunto de aula posterior.
